...
(function() {
  angular.module('CAPmodule').controller('ProgressNumbersCtrl', function(progressService) {
    ...
    numbersToRequest = ['percentScanned', 'pagesProcessed', 'volumesProcessed', 'metadataComplete', 'metadataCompleteChange', 'volumesProcessedChange', 'pagesProcessedChange'];
    self = this;
    getNum = function(num) {
...
(function() {
  angular.module('CAPmodule').controller('ProgressOverviewCtrl', function(progressService) {
    ...
    self = this;
    this.complete = {};
    numbersToRequest = ['percentScanned'];
    getNumbers = function() {
      var i, len, num, results;
      results = [];
      for (i = 0, len = numbersToRequest.length; i < len; i++) {
...
angular.module("../../templates/wordclouds.tpl.jade", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("../../templates/wordclouds.tpl.jade",
    "<div ng-controller=\"WordcloudsCtrl as wc\" class=\"wordclouds-container\"><div class=\"container-generic-tan wordclouds-title-container\"><a href=\"/\" class=\"container-title-logo\"><img src=\"./assets/img/logo-medium-blue.svg\" class=\"ftl-logo\"></a><h1 class=\"text-center\"> <span>CASELAW ACCESS PROJECT </span></h1><h4 class=\"text-center\">Explore the wordclouds!<br><span class=\"blue-text\"> <a href=\"http://librarylab.law.harvard.edu/projects/caselaw-access-project\" target=\"_blank\" class=\"read-more\">read more...</a></span></h4></div><div class=\"container-generic\"><div class=\"col-sm-4\"><div style=\"margin-bottom: 30px; position: relative;\" class=\"btn-group col-centered\"><span ng-if=\"wc.currentState\" data-toggle=\"dropdown\"><button type=\"button\" aria-haspopup=\"true\" aria-expanded=\"false\" class=\"blue-button dropdown-toggle\">{{wc.currentState.toUpperCase()}} WORDCLOUDS</button></span><span ng-if=\"!wc.currentState\" data-toggle=\"dropdown\"><button type=\"button\" aria-haspopup=\"true\" aria-expanded=\"false\" class=\"btn btn-default dropdown-toggle\">SELECT A STATE</button></span><ul class=\"dropdown-menu\"><li ng-repeat=\"state in wc.states\"> <a ng-click=\"wc.selectState(state)\">{{state}}</a></li></ul></div></div><div class=\"col-sm-8\"></div><div class=\"col-sm-12\"><ul><li ng-repeat=\"image in wc.images\" class=\"col-sm-4 wordcloud-item\"><label ng-if=\"image.indexOf('totals') &gt; -1\" class=\"col-sm-12\">{{image.substr(image.length - 10)}}</label><label ng-if=\"image.indexOf('totals') &lt; 0\" class=\"col-sm-12\">{{image.substr(image.length - 8)}}</label><a href=\"{{image}}\" class=\"col-sm-12\"><img src=\"{{image}}\" class=\"col-sm-12 wordcloud-images\"></a></li></ul></div></div></div>");
}]);
